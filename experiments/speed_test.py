...
실행    : python speed_test.py            (Python 3.8+, 표준 라이브러리만 사용)
          python speed_test.py --selftest (GUI 없이 지표 계산 검증)
...
관련 도구:
  - ../firmware/keyboard_glove/tap_calibration.py  연타 윈도우 캘리브레이션(실험 전 1회)
  - tv_remote_sprint.py 같은 과제의 커서 선택식 대조군(실험 D)
...
def save_record(record, jsonl_path, csv_path=CSV_PATH, logs_dir=LOGS_DIR):
    ...
    if new_file:   # 엑셀 호환을 위해 BOM은 파일 생성 시 1회만 기록
...
def _selftest():
    ...
    check("분해 닭(겹받침)", decompose_text("닭"), ["ㄷ", "ㅏ", "ㄹ", "ㄱ"])
    check("분해 의(겹모음)", decompose_text("의"), ["ㅇ", "ㅡ", "ㅣ"])
    check("분해 떡(쌍자음 1자모)", decompose_text("떡"), ["ㄸ", "ㅓ", "ㄱ"])
    check("MSD 동일", msd(list("abc"), list("abc")), 0)
    check("MSD 치환 1", msd(list("사과"), list("샤과")), 1)
    check("MSD 빈 문자열", msd(decompose_text("사과"), []), 5)

    m = compute_metrics([{"target": "사과", "typed": "사과"}], 60.0)
    check("지표 음절", m["syllables"], 2)
    check("지표 자모", m["jamo"], 5)
    check("지표 CPM", m["cpm_syl"], 2.0)
    check("지표 WPM", m["wpm"], 1.0)
    check("지표 오류율 0", m["msd_error_pct"], 0.0)
    m2 = compute_metrics([{"target": "사과", "typed": "샤과"}], 60.0)
    check("지표 오류율 20%", m2["msd_error_pct"], 20.0)
    m3 = compute_metrics([{"target": "사과", "typed": "사과"},
                          {"target": "학교", "typed": "학", "partial": True}], 60.0)
    check("partial 완성 단어에서 제외", m3["words_completed"], 1)
    check("partial 분량에는 포함", m3["syllables"], 3)
    check("partial 오류율에서 제외", m3["msd_error_pct"], 0.0)
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        rec = {
            ...
            "mode": "speed1min", "trial": 1, "started": "2026-07-12T10:00:00",
            "duration_s": 60, "words": [], "n_key_events": 42, "n_backspace": 3,
            **compute_metrics([{"target": "사과", "typed": "사과"}], 60.0),
            "events": [],
        }
        ...
        check("CSV wpm 열", float(rows[1][CSV_HEADER.index("wpm")]), rec["wpm"])
        check("JSONL 2줄",
              len((td / "t.jsonl").read_text(encoding="utf-8").strip().splitlines()), 2)
